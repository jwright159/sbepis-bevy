...
pub fn attack(
	...
	for hammer in hammers.iter() {
		commands.entity(hammer)
			.insert(InAnimation::default());
	}
}
...
pub fn animate_hammer(
	...
	for (hammer_pivot, mut transform, mut animation) in hammer_pivots.iter_mut() {
		let (hammer_head, _head) = hammer_heads.iter().find(|(_head_ent, head)| head.pivot == hammer_pivot).expect("Hammer pivot found without hammer head");
		...
		let angle = match time {
			0.0..0.2 => {
				...
				time.map_range(0.0..0.2, 0.0..(PI * 0.5)).cos().map_range(0.0..1.0, (-PI * 0.5)..0.0)
			},
			0.2..0.5 => {
				commands.entity(hammer_head).remove::<CanDealDamage>();
				time.map_range(0.2..0.5, 0.0..PI).cos().map_range(-1.0..1.0, 0.0..(-PI * 0.5))
			},
			_ => {
