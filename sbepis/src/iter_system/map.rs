...
pub trait Iterate<A: System, B: System> {
	type In;
...
pub struct IteratorSystem<Func, A, B> {
...
impl<Func, A, B> IteratorSystem<Func, A, B> {
...
impl<A, B, Func> System for IteratorSystem<Func, A, B>
where
	Func: Iterate<A, B> + 'static,
	A: System,
	B: System,
{
	type In = Func::In;
...
unsafe impl<A, B, Func> ReadOnlySystem for IteratorSystem<Func, A, B>
where
	Func: Iterate<A, B> + 'static,
	A: ReadOnlySystem,
	B: ReadOnlySystem,
...
impl<Func, A, B> Clone for IteratorSystem<Func, A, B>
where
	A: Clone,
	B: Clone,
{
...
pub type IterSystem<SystemA, SystemB> = IteratorSystem<Iter, SystemA, SystemB>;

#[doc(hidden)]
pub struct Iter;

impl<A, B, I, II, IIR> Iterate<A, B> for Iter
where
	A: System<Out = I>,
	B: System<In = II, Out = IIR>,
	I: IntoIterator<Item = II>,
{
	...
	fn combine(
		...
		let value = a(input);
		value.into_iter().map(b).collect()
	}
}

pub trait IteratorSystemTrait<In, Out, Marker>: IntoSystem<In, Out, Marker>
where
	Out: IntoIterator,
{
	fn iter_map<B, Final, MarkerB>(self, system: B) -> IterSystem<Self::System, B::System>
	where
		B: IntoSystem<Out::Item, Final, MarkerB>,
	{
		let system_a = IntoSystem::into_system(self);
		let system_b = IntoSystem::into_system(system);
		let name = format!("Iter({}, {})", system_a.name(), system_b.name());
		IterSystem::new(system_a, system_b, Cow::Owned(name))
	}
}
impl<T, In, Out, Marker> IteratorSystemTrait<In, Out, Marker> for T
where
