...
pub trait Done<A: System> {
	type In;

	fn combine(input: Self::In, a: impl FnOnce(A::In) -> A::Out);
}

pub struct DoneSystem<Func, A> {
...
impl<A, Func> System for DoneSystem<Func, A>
where
	Func: Done<A> + 'static,
	A: System,
{
...
unsafe impl<A, Func> ReadOnlySystem for DoneSystem<Func, A>
where
	Func: Done<A> + 'static,
	A: ReadOnlySystem,
{
...
impl<Func, A> Clone for DoneSystem<Func, A>
where
	A: Clone,
{
	fn clone(&self) -> Self {
...
pub type DonSystem<SystemA> = DoneSystem<Don, SystemA>;

#[doc(hidden)]
pub struct Don;

impl<A, I> Done<A> for Don
where
	A: System<Out = I>,
	I: IntoIterator,
{
...
pub trait DoneSystemTrait<In, Out, Marker>: IntoSystem<In, Out, Marker>
where
	Out: IntoIterator,
{
	fn iter_done(self) -> DonSystem<Self::System> {
		let system_a = IntoSystem::into_system(self);
		let name = format!("FSome({})", system_a.name());
		DonSystem::new(system_a, Cow::Owned(name))
	}
}
impl<T, In, Out, Marker> DoneSystemTrait<In, Out, Marker> for T
where
